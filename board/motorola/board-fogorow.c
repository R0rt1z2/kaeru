...
void cmd_flash(const char* arg, void* data, unsigned sz) {
    ...
    uint32_t addr = SEARCH_PATTERN(LK_START, LK_END, 0xB5F8, 0x4605, 0x4C3E, 0x460F);
    if (addr) {
...
void cmd_erase(const char* arg, void* data, unsigned sz) {
    ...
    uint32_t addr = SEARCH_PATTERN(LK_START, LK_END, 0xE92D, 0x43F0, 0x4604, 0x4D99);
    if (addr) {
...
void board_early_init(void) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF7FF, 0xFA72, 0xF8DF, 0x15D0);
    if (addr) {
        printf("Found cmd_flash_register at 0x%08X\n", addr);
        NOP(addr, 2);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF7FF, 0xFA68, 0xF8DF, 0x15C4);
    if (addr) {
    ...
    // present.
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF7FF, 0xF945, 0xF8DF, 0x145C);
    if (addr) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xB538, 0xF048, 0xFE17, 0xB110);
    if (addr) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xB508, 0xF7FF, 0xFFFB, 0xB908);
    if (addr) {

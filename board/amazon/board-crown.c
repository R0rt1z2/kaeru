...
void cmd_help(const char *arg, void *data, unsigned sz) {
    struct fastboot_cmd *cmd = (struct fastboot_cmd *)0x4BD871EC;
...
void board_early_init(void) {
    ...
    NOP(0x4BD27D02, 2); // => FASTBOOT mode...
    NOP(0x4BD12046, 2); // => FASTBOOT mode...
    NOP(0x4BD27CE0, 2); // => FACTORYRESET mode...
}
...
void board_late_init(void) {
    ...
    if (get_bootmode() == BOOTMODE_FASTBOOT) {
        ...
        NOP(0x4BD285DA, 2); // fastboot flash
        NOP(0x4BD28612, 2); // fastboot reboot
        NOP(0x4BD28624, 2); // fastboot reboot-bootloader

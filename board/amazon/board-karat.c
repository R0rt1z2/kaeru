...
static bool mem_region_find(const char *name, uint32_t *base, uint32_t *size) {
...
static const char *next_token(const char *p, char *out, size_t len) {
    while (*p == ' ') {
        p++;
    }
...
static void mem_hexdump(uint32_t addr, uint32_t len) {
    ...
    for (uint32_t off = 0; off < len; off += 8) {
        ...
        for (i = 0; i < n; i++) {
            used += npf_snprintf(line + used, sizeof(line) - used, " %02x",
                                 p[off + i]);
        }
...
static void cmd_oem_mem(const char *arg, void *data, unsigned sz) {
    char op[8];
    char tok[40];
    char line[FASTBOOT_INFO_MAX + 1];
    ...
    const char *end;
    uint64_t parsed = parse_hex64(tok, &end);

    if (*end) {
        if (!mem_region_find(tok, &addr, &region_size)) {
            fastboot_fail("Address is neither a number nor a known region");
            return;
        }
    } else {
    ...
    if (strcmp(op, "read") == 0) {
        ...
        if (!len || len > 0x400) {
            npf_snprintf(line, sizeof(line), "Length must be 1 to 0x%X", 0x400);
            fastboot_fail(line);
            return;
        }
...
static void fastboot_init_hook(const char *) {
    ...
    fastboot_register("oem mem", cmd_oem_mem, 1);

//
// SPDX-FileCopyrightText: 2025 inscrutable <qiratdahaf@users.noreply.github.com>
//                         2025 Shomy 🍂 
...
void spoof_lock_state(void) {
    ...
    if (addr) {
        ...
        NOP(addr + 0x6B8, 2);  // "not support on security" call
        NOP(addr + 0x6F8, 2);  // "not allowed in locked state" call
...
void board_early_init(void) {
    printf("Entering early init for Infinix X6812\n");
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF035, 0xF8B2, 0x6823, 0x3B01);
    if (addr) {
...
void board_late_init(void) {
    printf("Entering late init for Infinix X6812\n");

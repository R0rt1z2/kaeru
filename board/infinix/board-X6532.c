...
void board_late_init(void) {
    ...
    // Disables the warning shown during boot when the device is unlocked
    // and the dm-verity state is corrupted. This behaves like the previous
    // lock state warnings, visual only, with no real impact.
    //

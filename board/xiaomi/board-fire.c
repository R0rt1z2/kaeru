...
#define CMDLINE1_ADDR 0x4C579FE0
#define CMDLINE2_ADDR 0x4C57A7E4
...
static void spoof_lock_state(void) {
    ...
    // uses.
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF04B, 0xFD66, 0x4604, 0xB980);
    if (addr) {
        printf("Found seccfg_get_lock_state call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)get_lock_state, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF000, 0xFA0C, 0xB9B8, 0x9B02);
    if (addr) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0x4C92, 0xE92D, 0x4880, 0xB08D);
    if (addr) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF017, 0xFB5E, 0xF001, 0xF920);
    if (addr) {
...
void board_early_init(void) {
    printf("Entering early init for Xiaomi Redmi 12 4G\n");
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF03C, 0xFF57, 0x6823, 0x2000);
    if (addr) {
...
void board_late_init(void) {
    printf("Entering late init for Xiaomi Redmi 12 4G\n");
